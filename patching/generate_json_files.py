...
def get_default_resource_path():
    game_path = None
    if platform == "linux" or platform == "linux2":
        game_path = expanduser("~/Steam/steamapps/common/dota 2 beta/game/dota/scripts/npc")
    elif platform == "darwin":
        game_path = expanduser(
            "~/Library/Application Support/Steam/SteamApps/common/dota 2 beta/game/dota/scripts/npc")
    return game_path

def writeData(data, fname):
    with open(join('.', fname), 'w') as outfile:
        json.dump(data, outfile, indent = 4)

def processUnits():
    fName = open(join(get_default_resource_path(), 'npc_units.txt'), 'r')
...
def processHeroes():
    fName = open(join(get_default_resource_path(), 'npc_heroes.txt'), 'r')
...
def processAbilities():
    fName = open(join(get_default_resource_path(), 'npc_abilities.txt'), 'r')
